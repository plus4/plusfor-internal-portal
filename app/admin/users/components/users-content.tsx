...
export function UsersContent({ initialUsers, hasMoreUsers }: UsersContentProps) {
  console.error("[UsersContent] Component rendered with props:", {
    initialUsersLength: initialUsers?.length || 0,
    hasMoreUsers,
    firstUser: initialUsers?.[0] || null
  });
  ...
  useEffect(() => {
    console.error("[UsersContent] useEffect triggered");
    console.error("[UsersContent] Current users state:", {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`/api/admin/users?email=${encodeURIComponent(selectedUser.email)}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
